        ...
        if (!has_planted)
        {
            if (Normal_Plants.Choosing)
            {
                ...
                if ((Input.IsActionPressed("Left_Mouse") && !Public_Main.for_Android) || (Public_Main.for_Android && Is_Double_Click))
                {
                    card_parent_Button.Set_ColorRect_2(false);
                    Normal_Plants.Choosing = false;
                    if (on_lock_grid && ((In_Game_Main.Sun_Number >= card_parent_Button.sun && dock_area_2d.Down_Plant_List.Count == 0 && dock_area_2d.now_type[dock_area_2d.now_type.Count - 1] == 2) || Public_Main.debuging))
                    {
                        Is_Double_Click = false;
                        has_planted = true;
                        ...
                        if (dock_area_2d.now_type[dock_area_2d.now_type.Count - 1] == 2)
                        {
