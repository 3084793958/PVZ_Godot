...
public class Darts_Polevaulter_Zombies_Main : Normal_Zombies
{
    ...
    public async void Plants_Entered(Control_Area_2D area2D)
    {
            ...
            if (true/*plant_area2D.plants_type == "Normal"*/)
            {
                if (has_planted)
                {
                    if (plant_area2D.has_planted)
                    {
                        ...
                        if (!has_lose_pole)
                        {
                            if (!GetNode<AnimationPlayer>("Main/Main/Up").IsPlaying() && Top_Area_2D != null && !has_lose_Head && Top_Area_2D.plants_type != "Top")
                            {
                                Walk_Mode(false);
        ...
        if (area2D.Area2D_type == "Plants_Bullets" && has_planted && !is_jumping)
        {
    ...
    public override void _Process(float delta)
    {
        ...
        else
        {
            ...
            if (GetNode<Normal_Zombies_Area>("Main/Main/Zombies_Area").GlobalPosition.x < -1437 / 2)
            {
                this.Free();
                this.Remove_Zombies_Number();
            }
            if (!In_Game_Main.has_Lost_Brain && GetNode<Normal_Zombies_Area>("Main/Main/Zombies_Area").GlobalPosition.x < -20 && !has_lose_Head)
            {
            ...
            if (!eating && (GetNode<AnimationPlayer>("Main/Main/Walk1").IsPlaying() || GetNode<AnimationPlayer>("Main/Main/Walk2").IsPlaying()) && !On_Boom_Effect && !Is_Shining && !is_Lock_Ice)
            {
                    ...
                    if (Plants_Area_2D_List != null)
                    {
                        if (Plants_Area_2D_List.Count != 0)
                        {
                            ...
                            else
                            {
                                ...
                                else if (Top_Area_2D == null && !has_lose_Head && !is_jumping)
                                {
                                    eating = false;
                                    Walk_Mode(true);
                                    GetNode<AnimationPlayer>("Main/Main/Eating").Stop();
